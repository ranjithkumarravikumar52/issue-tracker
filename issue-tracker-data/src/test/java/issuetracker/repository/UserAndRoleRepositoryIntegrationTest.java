 ...
 *
 * <br>
 * <p>This class primarily tests the relationship between User and Role and their bi-directional mapping. In
 * addition, couple of methods on crud operation has been tested to validate our mapping of our entities.</p>
 */
...
public class UserAndRoleRepositoryIntegrationTest extends AbstractClassRepositoryTest {
    ...
    public void roleToUserObjectTreeNavigation() {
        ...
        Role role = roleRepository.findById(tester.getId())
                .orElse(null);
        assertNotNull(role);
        assertEquals(tester.getName(), role.getUserSet()
                .toArray(new User[0])[0].getRole()
                .getName());
    }
    ...
    /**
     * Since our mapping between user and role doesn't specify any cascade types, save operation doesn't cascade to
     * roles.
    ...
    public void whenUserAndRoleAreSaved_thenUserFindAllWorks() {
        ...
        userRepository.findAll()
                .forEach(userList::add); //shouldn't throw an exception
    ...
    public void whenAnUserIsDeleted_thenRoleShouldNotBeDeleted() {
        ...
        Role role = roleRepository.findById(developer.getId())
                .orElse(null);
        assertNotNull(role);
    ...
    /**
     * When user and role objects are saved and their corresponding direction is established, then calling findById
    ...
    /**
     * when an user is deleted then the expected behavior is user should be deleted however the associated role
     * object shouldn't.
     */
    @Test
